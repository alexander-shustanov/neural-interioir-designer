...
from model.ops import gen_conv, lrelu, batchnorm, gen_deconv, discrim_conv
...
def create_generator(generator_inputs, generator_outputs_channels, context=None):
    layers = []

    # encoder_1: [batch, 256, 256, in_channels] => [batch, 128, 128, ngf]
    with tf.variable_scope("encoder_1"):
        if context is not None:
            output = gen_conv(tf.concat([generator_inputs, context], axis=3), args.ngf)
        else:
...
def create_model(inputs, targets, context=None):
    with tf.variable_scope("generator"):
        out_channels = int(targets.get_shape()[-1])
        outputs = create_generator(inputs, out_channels, context)
